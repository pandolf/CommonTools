...
public:
  ...
  static void fitProjection(TH1D* h1_projection, TF1* gaussian, Float_t nSigma = 1.5, std::string option = "RQ", bool add=false);

  static void fitProjection_sameArea(TH1D* h1_projection, TF1* gaussian, TH1D* newhisto, Float_t percIntegral=0.9, const std::string& option="RQ", bool useMode=false);

  static void getTruncatedMeanAndRMS(TH1D* h1_projection, Float_t& mean, Float_t& mean_err, Float_t& rms, Float_t& rms_err, Double_t percentIntegral_MEAN=0.9, Double_t percentIntegral_RMS=0.68);
